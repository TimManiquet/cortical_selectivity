...
# -*-coding:utf-8 -*-
'''
Figures and stats for analyses about basic category motor task x brain
...
## Data
basic_motor_df = pd.concat([
    pd.read_pickle(os.path.join(basic_motor_data_dir, "preprocessed_data_part1.pkl")),
    pd.read_pickle(os.path.join(basic_motor_data_dir, "preprocessed_data_part2.pkl"))],
ignore_index=True)
...
trajectories = {
    category: np.concatenate([
        np.load(os.path.join(basic_motor_data_dir, f"target-{category}_img_trajectories_part1.npy")),
        np.load(os.path.join(basic_motor_data_dir, f"target-{category}_img_trajectories_part2.npy"))
    ])
    for category in categories
}

## Figure 3: motor movements & correlation with brain

# figure 3a: all trajectories of example participant

for ex_ppt in [64]:
    
    ex_data = basic_motor_df[basic_motor_df['ppt_number'] == ex_ppt]
    
    plt.figure(figsize=(1.5, 1.2))
    ax = plt.gca()
    for i, row in ex_data.iterrows():
        img = row['image']
        img_category = row['category']
    ...
    plt.savefig(os.path.join(figure_dir, f'figure_3a_ex-ppt-{ex_ppt}_trajectories.{figure_ext}'), bbox_inches='tight', dpi=300)
    # plt.show()
    plt.close()

    # figure 3b: all trajectories of example ppt per task

    for target_category in ['body', 'face', 'scene']:
        
        ex_target_data = basic_motor_df[(basic_motor_df['ppt_number'] == ex_ppt)&(basic_motor_df['target_category'] == target_category)]
        
        # plot xy trajectories per task
        
        plt.figure(figsize=(1.0, 1.1))
        ax = plt.gca()
        for i, row in ex_target_data.iterrows():
            img = row['image']
            img_category = row['category']
            x = row['scaled_x'][:max_t]
            y = row['scaled_y'][:max_t]
        ...
        ax.set_ylim(0, 1)
        ax.set_axis_off()
        plt.savefig(os.path.join(figure_dir, f'figure_3a_ex-ppt-{ex_ppt}_trajectories_target-{target_category}.{figure_ext}'), bbox_inches='tight', dpi=300)
        # plt.show()
        plt.close()
        
# Figure 3b: per task, x coordinates across time

# create a list of int from 0 to 799 randomly
random_img_indices = np.random.choice(len(images), size=185, replace=False)
for category, trajectory_data in trajectories.items():

    avg_trajectories = np.nanmean(trajectory_data, axis=1)

    f, ax = plt.subplots(figsize=(1.5, 0.9))
    for random_img_idx in random_img_indices:
        ...
        img_category = [c for c in categories if c in images[random_img_idx]][0]
        ax.plot(range(max_t), trajectory, color=palette[img_category], alpha=0.15)
    plt.savefig(os.path.join(figure_dir, f'figure_3b_avg_trajectories_{category}.{figure_ext}'), bbox_inches='tight', dpi=300)
    # plt.show()
    plt.close()

# figure 3c: correlation between brain activations and motor trajectories
...
for roi in rois:
    ...
    trajectory_data = trajectories[preferred_category]
    ...
    for category, img_indices in img_indices.items():
        ...
        for t in range(max_t):
            traj_values = np.nanmean(trajectory_data[img_indices, :, t], axis=1)
            brain_values = roi_data.iloc[img_indices].values            
            corr, pvalue = pearsonr(traj_values, brain_values)
            # corr, pvalue = pearsonr(traj_values, brain_values, alternative='greater')
            corrs.append(corr)
...
# Figure 3c: plot correlations over time
for roi in rois:
    ...
    plt.savefig(os.path.join(figure_dir, f'figure_3c_{roi}_trajectory_brain_correlation.{figure_ext}'), bbox_inches='tight', dpi=300)
    # plt.show()
    plt.close()